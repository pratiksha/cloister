...
def start_driver(driver_ip, bench_name, nprocs, npartitions, manager_name, worker_ips, aws_key, local):
    run_cmd_nonblock(driver_ip, "scripts/run-driver.sh %s %d %s %s %d" % (bench_name, nprocs, manager_name, worker_ips, npartitions), aws_key, local)
...
def main():
    ...
    parser.add_argument('-m', "--driver_name", type=str, default="servers/driver.txt",
                        help="Filename containing driver IP")
    ...
    parser.add_argument('-z', "--no_driver", action='store_true',
                        help="Run workers only (start driver manually)")
    
    args = parser.parse_args()

    server_names = read_ips(args.server_names)
    driver_name = read_ips(args.driver_name)[0]
    ...
    if args.action == 'experiment':
        ...
        # don't use this - SSH to driver node to start memory manager for experiments, instead
        ...
        if not args.no_driver:
            start_driver(driver_name, args.benchmark, args.nprocs, args.nprocs * len(worker_names), manager_name, ','.join(worker_names), args.aws_key, args.local)

        time.sleep(1000) # wait for end

    elif args.action == 'killall':
        kill_servers([driver_name], args.benchmark, args.aws_key)
        kill_servers(server_names, args.benchmark, args.aws_key)
        
    elif args.action == 'make':
        for name in server_names + [driver_name]:
        ...
        start_driver(manager_name, args.benchmark, args.nprocs, args.nprocs * len(worker_names), manager_name, ','.join(worker_names), args.aws_key, args.local)
